...
if (length(args) == 4){
  selected_anchors = fread(args[4])
} else{
  anchor_sample_fraction_cutoff = args[4] # the cutoff for the minimum fraction of samples for each anchor, suggested 0.4
  num_anchors_for_supervised_test = args[5] # maximum number of anchors to be tested example, suggested 20000
}
...
for (counter in 0:127){
  system(paste("/oak/stanford/groups/horence/Roozbeh/software/R-NOMAD/bin/satc_dump --anchor_list ", paste(directory, "/",run_name,"_supervised_metadata/", "selected_anchors_GLMnet.txt ", sep = ""), directory, "result.bin", counter, ".satc ", directory, "/",run_name,"_supervised_metadata/", "satcOut",counter,".tsv ", sep = ""))
}
